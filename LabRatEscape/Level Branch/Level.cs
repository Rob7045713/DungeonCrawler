...
namespace LabRatEscape
{
    ...
    public class Level
    {
        ...
        public static GameValue currentLevel = new GameValue(0, 0, -1, 1);
        public static Dictionary<String, Block> possibleBlocks = new Dictionary<string, Block>();
        public static Block defaultBlock;
        static ArrayList levelPaths = new ArrayList();
        public static List<Portal> portalsEntrance = new List<Portal>();
        public static List<Portal> portalsExit = new List<Portal>();
        ...
        public static void Reset()
        {
            ...
            portalsEntrance.Clear();
            portalsExit.Clear();
            headerDictionary.Clear();
            GetInstance().LoadLevel(0);
        }
        ...
        private static void BuildHeaderDictionary()
        {
            ...
            for (int i = 0; i < header.Count; i++)
            {
                ...
                {
                    Item item = ItemManager.possibleItems[objectID].CopyItemAt(new Vector2(0,0));
                    AddToHeaderDictionary(objectChar, item);
                }
                if (EnemyManager.possibleEnemies.ContainsKey(objectID))
                {
                    Enemy enemy = EnemyManager.possibleEnemies[objectID].CopyEnemyAt(new Vector2(0, 0));
                    AddToHeaderDictionary(objectChar, enemy);
                }
                if (possibleBlocks.ContainsKey(objectID))
                {
                    Block block = possibleBlocks[objectID].Clone();
                    if (!s.Equals(""))
                    {
                        object[] parameters = { s };
                        block.effects.Add(new Effect(new GameAction(PlayerManager.players[0],
                        PlayerManager.players[0].GetType().GetMethod("setMessage"), parameters)));
                    }
                    AddToHeaderDictionary(objectChar, block);
                    block.setMatchID(objectChar);
        ...
        private void createMap()
        {
            ...
            if (!hasEnterance)
            {
                Console.WriteLine("Level.createMap() : No entrance");
            }
        ...
        /// <summary>
        /// sets up links between entrance and exit portals with matching matchID values
        /// </summary>
        private void SetUpPortals()
        {
            foreach (Portal entrance in portalsEntrance)
            {
                String matchIDEntrance = entrance.getMatchID();
                foreach (Portal exit in portalsExit)
                {
                    String matchIDExit = exit.getMatchID();
                    if (matchIDEntrance.ToUpper().Equals(matchIDExit.ToUpper()))
                    {
                        entrance.Link(exit);
                    }
                }
        ...
        {
            ItemManager.SaveInventories();
            PlayerManager.SavePlayers();
            if (currentLevel.IsMax())
            {
                Engine.Menu(Engine.GetInstance().winGameMenu);
                currentLevel.Reset();
            }
            else
            {
                Engine.Menu(Engine.GetInstance().winLevelMenu);
                currentLevel.Add(1);
            }
        ...
        {
            ItemManager.RevertInventories();
            PlayerManager.RevertPlayers();
            GetInstance().LoadLevel((int)currentLevel.getValue());
        }
