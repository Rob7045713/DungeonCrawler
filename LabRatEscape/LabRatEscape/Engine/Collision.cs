...
namespace LabRatEscape
{
    ...
    public class Collision
    {
        ...
        public Collision(Sprite actor, Sprite prop)
        {
            this.actor = actor;
            this.prop = prop;
            actorRectangle = new Rectangle((int)actor.position.X - actor.frameWidth / 2, (int)actor.position.Y - actor.frameHeight / 2, actor.frameWidth, actor.frameHeight);
            propRectangle = new Rectangle((int)prop.position.X - prop.frameWidth / 2, (int)prop.position.Y - prop.frameHeight / 2, prop.frameWidth, prop.frameHeight);
            isColliding = false;
            isOverlapping = false;
            DoCollision();
        }
